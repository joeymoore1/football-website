...
class StatsController @Inject()(val controllerComponents: ControllerComponents,
                                resultsService: ResultsService, profileService: ProfileService) extends BaseController {
  ...
  def index(): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    ...
    Ok(views.html.stats(goalScorerList, assistsList))
  }

  def playerStatsList(players:List[Player]):List[playerStats] = {
    players.map(l => playerStats(l, profileService.getGoalsForPlayer(l), profileService.getAssistsForPlayer(l), profileService.getAppearances(l)))
  }
}
